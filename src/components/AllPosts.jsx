...
export default function AllPosts({id, username, pictureUrl, link, article, urlTitle, urlDescription, urlImage, userId, sessionUserId, refreshAxios, setRefreshAxios}){
    ...
    let loadingAnimation = (
        ...
        setLoadingEdit(true)
        const request = axios.put(/* `https://projeto17-linkr-backend.herokuapp.com/post/${id}` */ `http://localhost:5000/post/${id}`,dados, config);
        ...
        const request = axios.delete(`http://localhost:5000/post/${id}`, config);
        request.then((response) => {
            console.log(response)
    ...
    return(
      <IconContext.Provider value={{ color: "#FFFFFF", fontSize:"16px"}}>
        ...
        >
          <ContainerModal>
            {loadingDelete ? loadingAnimation :
            <>
              <h4 >Are you sure you want to delete this post?</h4>
              <div className="modalButtons">
                  <button className="back"  onClick={showModal} >No, go back</button>
                  <button type="submit" className="delete"  onClick= {deletePost} > Yes, delete it</button>
              </div>
            </>  
            }
            </ContainerModal>
